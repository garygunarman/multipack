...
<head>	
	...
	<script type="text/javascript" src="<?php echo $prefix;?>script/color.js"></script>
	...
	<link rel="stylesheet" type="text/css" href="<?php echo $prefix;?>style/color.css" />
...
<body onload="initialization()">
	...
	<div id="page_title_bar">
		<div id="page_title_bar_fill">
			<div id="page_title">Color</div>
			<div id="button_bar">
				<a href="<?php echo $prefix;?>color/add_color"><input type="button" id="add_color_button" class="add_button image_button h26_button green_button button" value="Add Color"></a>
				<!--<input type="submit" class="h26_button green_button button" value="Save &amp; Continue Edit">-->
			</div>
		</div>
	</div>
	<form id="color_form" method="post" action="update.php" enctype="multipart/form-data">
	<?php
	...
	echo '<input type="hidden" name="url" id="url" class="hidden" value="../color" />';
	...
	<div class="admin_table">
		...
		<div class="admin_header_row">
			...
			<div class="left_ admin_header" id="color_image_header" ></div>
			<div class="admin_header" id="color_name_header" >Name</div>
		</div>
		...
		if (mysql_num_rows($get_list)!=null){
			...
			for ($row=1;$row<=mysql_num_rows($get_list);$row++){
				...
				echo '<li list_id="'.$get_list_array["color_id"].'">';
				if ($row%2==1){
 				...
 				echo '<div class="left_ table_field check_field"><input type="checkbox" name="id_[]" value="'.$get_list_array["color_id"].'" id="check_'.$row.'" class="" onmouseover="downCheck()" onmouseout="upCheck()" onclick="selectRowCheck('.$row.')"/></div>';

				echo '<input type="hidden" name="color_order_'.$row.'" value="'.$get_list_array["color_order"].'" />';

				echo '<div class="hidden left_ table_field color_id_field">'.$get_list_array["color_id"].'</div>';
				
				
				echo '<div class="left_ table_field color_image_field"><img class="color_image" src="'.$prefix.'../'.$get_list_array["color_image"].'" alt="'.$get_list_array["color_name"].'" /></div>';
				
				echo '<div class="table_field color_name_field"><a class="table_link"  href="detail.php?color_id='.$get_list_array["color_id"].'&color_name='.$get_list_array["color_name"].'">'.$get_list_array["color_name"].'</div>';
