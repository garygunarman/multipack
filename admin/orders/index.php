...
<head>	
	...
	<script type="text/javascript" src="<?php echo $prefix;?>script/orders.js"></script>
...
<body onload="initialization(),adminPanelFix()">
	...
	else if ($sort_by=="order_confirm_amount"){
		$sort_by ="(order_confirm_amount+1)";
	}
	else if ($sort_by=="order_confirm_amount DESC"){
		$sort_by ="(order_confirm_amount+1) DESC ";
	}
	...
	WHERE (order_status!='expired') AND ($search)
	ORDER BY $sort_by	
	",$con);
	...
	if ($_GET["user_id"]!=null){
		...
		$get_list_full =mysql_query("
		SELECT * from tbl_order AS order_ INNER JOIN tbl_user_purchase AS user
		ON order_.order_id = user.order_id
		WHERE (order_status!='expired') AND user_id = '$user_id' AND ($search)
		ORDER BY $sort_by	
		",$con);
	}
	...
	$total_page = ceil($total_query / $query_per_page);
		...
		SELECT *, DATE(order_date) AS date from tbl_order
		WHERE (order_status!='expired') AND ($search)
		ORDER BY $sort_by
		...
		if ($_GET["user_id"]!=null){
			...
			$get_list =mysql_query("
			SELECT *, DATE(order_date) AS date from tbl_order	AS order_ INNER JOIN tbl_user_purchase AS user
			ON order_.order_id = user.order_id
			WHERE (order_status!='expired') AND user_id = '$user_id' AND ($search)
			ORDER BY $sort_by
			LIMIT $first_record, $query_per_page
		...
		if ($sort_by=="(order_confirm_amount+1)"){
			$sort_by ="order_confirm_amount";
		}
		else if ($sort_by=="(order_confirm_amount+1) DESC"){
			$sort_by ="order_confirm_amount DESC ";
		}
		
		echo '<input type="hidden" name="url" id="url" class="hidden" value="../orders" />';
	...
	<div class="admin_table">
		...
		<div class="admin_header_row">
			...
			?>
			" id="order_no_header" onclick="sortBy('order_number')">Order No.
				<?php if ($_GET["sort_by"]=="order_number"){?>
					<div class="header_arrow_down"></div>
				<?php }?>
				<?php if ($_GET["sort_by"]=="order_number DESC"){?>
			...
			?>
			" id="date_header" onclick="sortBy('order_date')">Date
			...
			?>
			" id="bank_header" onclick="sortBy('order_confirm_bank')">Bank
			...
			?>
			" id="amount_header" onclick="sortBy('order_confirm_amount')">Amount 
			...
			?>
			" id="name_header" onclick="sortBy('order_confirm_name')">Name
		...
		<div class="left_ table_field check_field"><a href="../orders"><input type="submit" class="right_ black_tiny_button tiny_button reset_button" id="reset_button" value="" /></a></div>
		...
		if (mysql_num_rows($get_list)!=null){
			for ($row=1;$row<=mysql_num_rows($get_list);$row++){
				...
				echo '<div class="left_ table_field order_no_field"><a class="table_link"  href="detail.php?order_number='.$get_list_array["order_number"].'">'.$get_list_array["order_number"].'</a></div>';
